...
const EditDealDialog = ({ open, onOpenChange, deal, onDealUpdated }: EditDealDialogProps) => {
  ...
  // Populate form with deal data when dialog opens
  useEffect(() => {
    if (open && deal) {
      setFormData({
        deal_name: deal.deal_name || '',
        startup_name: deal.startup_name || '',
        investor_name: deal.investor_name || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await adminApiService.updateDeal({
        id: deal.id,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Update the deal information. All fields marked with * are required.
          </DialogDescription>
        </DialogHeader>
